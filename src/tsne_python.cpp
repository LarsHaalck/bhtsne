...
PyObject* run(bp::object obj, int noDims, double perplexity, double theta,
        int maxIt, int stopLyingIt, int momentumSwitch, int numThreads)
{
    if (numThreads > 0)
    ...
    tsne::TSNE::run(vec, dims[0], dims[1], Y, noDims, perplexity, theta, -1, false,
            maxIt, stopLyingIt, momentumSwitch);
...
BOOST_PYTHON_MODULE(tsne_omp)
{
    ...
    def("run", run, bp::default_call_policies(), (bp::arg("obj"), bp::arg("noDims"),
