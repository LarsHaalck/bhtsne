...
std::shared_ptr<std::vector<double>> np_to_std_vec(bp::object obj,
        std::vector<npy_intp>& dims)
{
    ...
    return std::make_shared<std::vector<double>>(arr_value_ptr,
            arr_value_ptr + dims[0] * dims[1]);
}

void write_results(PyObject* obj, std::shared_ptr<std::vector<double>> res,
        ...
        int num_threads)
{
    ...
    auto Y = std::make_shared<std::vector<double>>(dims[0] * no_dims);
...
BOOST_PYTHON_MODULE(tsne_cpp)
{
    ...
    def("tsne", tsneRun, bp::default_call_policies(), (bp::arg("obj"),
                bp::arg("no_dims")=2, bp::arg("perplexity")=30.0, bp::arg("theta")=0.5,
